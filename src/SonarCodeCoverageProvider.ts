...
import SonarReport from "./istanbul/sonar";
...
export default class SonarCodeCoverageProvider extends V8CoverageProvider {
  ...
  declare testExclude: InstanceType<TestExclude>;

  // @ts-ignore
  declare coverageFiles = new Map<ProjectName, CoverageFilesByTransformMode>();
  declare coverageFilesDirectory: string;
  ...
  initialize(ctx: Vitest) {
    ...
    // this.options = super.resolveOptions();
  }
  ...
  async reportCoverage({ allTestsRun }: ReportContext = {}) {
    ...
    // create reports for each reporter
      ...
      if (this.options.thresholds.autoUpdate && allTestsRun) {
        ...
        this.updateThresholds({
          ...
          configurationFile: {
            ...
            read: () =>
              configModule.exports.default.$type === "function-call"
  ...
  private async getUntestedFiles(testedFiles: string[]): Promise<RawCoverage> {
    ...
    for (const chunk of toSlices(
      ...
      const coverages = await Promise.all(
        chunk.map(async (filename) => {
          const { source } = await this.getSources(
            filename.href,
            transformResults
          );
  ...
  private async getSources(
    ...
    if (!map) return { source: code || sourcesContent };
 ...
 */
function removeViteHelpersFromSourceMaps(
  source: string | undefined,
  map: EncodedSourceMap
) {
  if (!source || !source.match(VITE_EXPORTS_LINE_PATTERN)) return map;
...
function normalizeTransformResults(
  ...
  for (const [key, value] of fetchCache.entries()) {
    ...
    if (!normalized.has(cleanEntry)) normalized.set(cleanEntry, value.result);
  }
